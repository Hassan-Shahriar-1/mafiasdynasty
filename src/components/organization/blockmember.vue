<template>
        ...
        <div class="row Gp0m0 " v-else>
            <div class="col-3 Gp0m0 "  v-for="(blk,i) in blklst" :key="i">
                <div class="m-1  blksdo2" @click="show(blk.id,blk.name)">
                    ...
                    <div class="dtlsfnt bMbg5 text-white"><h5 class="px-1  sizv20 ">{{blk.name}}
                        <h6 class="sizv13">LVL: {{blk.lvl}}</h6></h5>
                    </div>
                </div>
            </div>                     
        </div>
    ...
    <modal name="mdlMempsts"  :width='300'   :isAutoHeight="true"   @before-open="beforeOspen" class="mblr" >
        <div class="row Gp0m0 tmibg  bdr3 p-3 blksdout rounded" id='gtmhyHt'>
            <div class="col-12 p-0 pb-2">
                <h4 class="subtitlefnt text-center">{{unm}}</h4> <br>
                <h6 class="dtlsfnt text-center"></h6>
            </div>
            <div class="col-12 Gp0m0 pr-1" >
                <div class="btn nBtn btn-lg text-center btnbfnt btn-block bMbg5 sizv35 " @click="vwprf(uuid)">View Profile</div>
                <div class="btn nBtn btn-lg text-center btnbfnt btn-block sizv35" @click="unblk(uuid)" >Unblock</div>
            </div>                        
...
export default {
    ...
    methods: {
        ...
        vwprf(id){//recive id then redirec to any user profile///
