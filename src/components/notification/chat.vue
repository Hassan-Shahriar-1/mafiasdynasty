        ...
        <div class=" Gp0m0 bw100 rounded " style="width:100%; border-radious:1px solid red; position:sticky;">         
            <div class="input-group Gp0m0 ">
                ...
                <div class="input-group-append " @click="insrtMsg()">
                    <span class="input-group-text px-3 send_btn blkgry text-white border-0"><i class="fas fa-location-arrow"></i></span>
                </div>
            </div> 
        </div>
                ...
                <div class="d-flex justify-content-start" v-if="MsgsLt.pstn==='l'">
                    <img class="card-img rounded-circle img_cont_msg " src="../../assets/img/logo.png" alt="">
                        <div class="msg_cotainer sizv20 dtlsfnt w-100">
                           ...
                           <h6 class="mt-2 mlr2 siz13 float-right msg_time text-white"> <i class="fas fa-clock"></i>  
                            <vue-moments-ago prefix="" suffix="ago" :date="MsgsLt.tm" lang="en" /> 
                           </h6>
                        </div>
                </div>       
                <div class="d-flex justify-content-end" v-else style="word-break:break-all;">
                    <div class="msg_cotainergsp sizv20 dtlsfnt w-100 text-right">
                            <div class="mb-2 ">{{ MsgsLt.msg }}</div>
                        <h6 class="text-white text-right"> <i class="fas fa-clock"></i>  
                            <vue-moments-ago prefix="" suffix="ago" :date="MsgsLt.tm" lang="en" /> 
                       </h6>
                    </div>
...
export default{
    ...
    created() {
         this.axios.get('mp/chkmafia/'+this.$props.id).then(res=>{////checking if user exist or not/////
         console.log('tag',res.data)
             if(res.sts=='you'){///for own msg
                 this.$router.push('/game/home/gtns');
             }else if(res.sts=='errmsg'){///for usr not found
                 this.$router.push('/game/home/gtns');
             }else if(res.sts=='ok'){///if user found then get all msg
                    this.usrnm=res.data.name;
                    this.oid=res.data.uid;
                    if(res.data.msglstd !='we didnt find any conversation with this user'){
