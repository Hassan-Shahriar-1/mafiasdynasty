<template >
    <div class="signup-form m-0 ptnbg " style=" height:100vh;overflow-y:hidden;">
        ...
        <div class="pt-3">
            <div class="form-control rounded p-0">
                <div class="position-relative has-icon-right">
                    <input class="form-control form-control-lg text-left rounded-0 " v-model="email"  type="email" placeholder="Email" required>
                    <div class="form-control-position">
                        <i class="fas fa-envelope-open text-dark"></i>
                    </div>
                </div>
                <div class="position-relative has-icon-right">
                    <input class="form-control form-control-lg text-left rounded-0" v-model="password" id="myInput" autocomplete="ppswd"  placeholder="Password" :type="passwrdfttp" >
        ...
        <div class="text-center  text-white dtlsfnt">
             <h3><router-link class="text-info siz20" :to="{name:'forget'}">Forget Password? </router-link>  | <router-link class="text-info siz20" :to="{name:'signup'}" >Sign Up Here</router-link></h3> 
         </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        logindata: function() {
             ...
             this.$mgo.gp('/auth/login', {'email':this.email,'password':this.password}, (response)=>{                 
                    ...
                    switch(response.sts){
                        ...
                        case 'ok' : 
                            if(response.token){
                               // alert('I m fine');
                                this.usrtkn =  window.localStorage.setItem("Usrtkn", response.token);
                                /* window.localStorage.setItem("UsrAuth", 'Bearer '+this.usrtkn); */
                              
                                this.axios.defaults.headers.Authorization = 'Bearer '+window.localStorage.getItem("Usrtkn");
